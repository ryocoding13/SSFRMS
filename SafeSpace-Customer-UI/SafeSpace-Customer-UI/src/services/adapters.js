...
export function mapMoveOutFromApi(row) {
  return {
    inspection_id: row.inspectionId,
    contract_id: row.contractId,
    unit_number: row.unitNumber,
    scheduled_at: row.scheduledAt,
    requested_date: row.scheduledAt?.slice(0, 10) || null,
    status: row.status,
    notes: row.notes || "",
...
export function mapAppointmentFromApi(row) {
  return {
...
export function mapNotificationFromApi(row) {
  return {
